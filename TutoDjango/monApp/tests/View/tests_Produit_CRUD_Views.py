...
class ProduitCreateViewTest(TestCase):
    ...
    def test_produit_create_view_get(self):
        response = self.client.get(reverse('creer_produit')) # Utilisation du nom de l'URL
    ...
    def test_produit_create_view_post_valid(self):
        data = { "intituleProd": "ProduitPourTestCreation", 'prixUnitaireProd': '59.99', 'dateFabricationProd': '2025-09-09' }
        response = self.client.post(reverse('creer_produit'), data)
...
class ProduitDetailViewTest(TestCase):
    def setUp(self):
        self.produit = Produit.objects.create(intituleProd="ProduitPourTestDetail", prixUnitaireProd=59.99, dateFabricationProd='2025-09-09')

    def test_produit_detail_view(self):
        response = self.client.get(reverse('detail_produit', args=[self.produit.refProd]))
        self.assertEqual(response.status_code, 200)
...
class ProduitUpdateViewTest(TestCase):
    def setUp(self):
        self.produit = Produit.objects.create(intituleProd="ProduitPourTestUpdate", prixUnitaireProd=59.99, dateFabricationProd='2025-09-09')
        self.user = User.objects.create_user(username='testuser', password='secret')
        self.client.login(username='testuser', password='secret')
    
    def test_produit_update_view_get(self):
        response = self.client.get(reverse('change_produit', args=[self.produit.refProd]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_view_post_valid(self):
        ...
        data = {'intituleProd': 'ProduitPourTestAfterUpdate', 'prixUnitaireProd': '59.99', 'dateFabricationProd': '2025-09-09'}
        response = self.client.post(reverse('change_produit', args=[self.produit.refProd]), data)
...
class ProduitDeleteViewTest(TestCase):
    def setUp(self):
        self.produit = Produit.objects.create(intituleProd="ProduitPourTesDelete", prixUnitaireProd=59.99, dateFabricationProd='2025-09-09')
        self.user = User.objects.create_user(username='testuser', password='secret')
        self.client.login(username='testuser', password='secret')
    
    def test_produit_delete_view_get(self):
        response = self.client.get(reverse('delete_produit', args=[self.produit.refProd]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'monApp/delete_produit.html')
    
    def test_produit_delete_view_post(self):
        response = self.client.post(reverse('delete_produit', args=[self.produit.refProd]))
