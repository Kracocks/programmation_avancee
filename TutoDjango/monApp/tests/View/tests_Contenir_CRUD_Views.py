...
class ContenirCreateViewTest(TestCase):
    ...
    def test_contenir_create_view_get(self):
        response = self.client.get(reverse('cntnr_crt', args=[self.rayon.idRayon]))
    ...
    def test_contenir_create_view_post_valid(self):
        produit = Produit.objects.create(
            ...
            prixUnitaireProd=10,
            dateFabricationProd='2025-09-12'
        )
        data = { 
            "produits": produit.refProd,
            "quantite": 3
        }
        response = self.client.post(reverse('cntnr_crt', args=[self.rayon.idRayon]), data)
...
class ContenirUpdateViewTest(TestCase):
    ...
    def test_contenir_update_view_get(self):
        response = self.client.get(reverse('cntnr_upt', args=[self.rayon.idRayon, self.contenir.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_contenir_update_view_post_valid(self):
        ...
        response = self.client.post(reverse('cntnr_upt', args=[self.rayon.idRayon, self.contenir.id]), data)
        self.assertEqual(response.status_code, 302)
...
class ContenirDeleteViewTest(TestCase):
    ...
    def test_contenir_delete_view_get(self):
        response = self.client.get(reverse('cntnr_dlt', args=[self.rayon.idRayon, self.contenir.id]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'monApp/delete_contenir.html')

    def test_contenir_delete_view_post(self):
        response = self.client.post(reverse('cntnr_dlt', args=[self.rayon.idRayon, self.contenir.id]))
        self.assertEqual(response.status_code, 302)
        self.assertFalse(Contenir.objects.filter(id=self.contenir.id).exists())
        self.assertRedirects(response, reverse('detail_rayon', args=[self.rayon.idRayon]))