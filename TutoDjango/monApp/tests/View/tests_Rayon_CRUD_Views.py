...
class RayonCreateViewTest(TestCase):
    ...
    def test_rayon_create_view_get(self):
        response = self.client.get(reverse('creer_rayon')) # Utilisation du nom de l'URL
    ...
    def test_rayon_create_view_post_valid(self):
        ...
        response = self.client.post(reverse('creer_rayon'), data)
...
class RayonDetailViewTest(TestCase):
    ...
    def test_rayon_detail_view(self):
        response = self.client.get(reverse('detail_rayon', args=[self.rayon.idRayon]))
        self.assertEqual(response.status_code, 200)
...
class RayonUpdateViewTest(TestCase):
    ...
    def test_rayon_update_view_get(self):
        response = self.client.get(reverse('change_rayon', args=[self.rayon.idRayon]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_view_post_valid(self):
        ...
        response = self.client.post(reverse('change_rayon', args=[self.rayon.idRayon]), data)
...
class RayonDeleteViewTest(TestCase):
    ...
    def test_rayon_delete_view_get(self):
        response = self.client.get(reverse('delete_rayon', args=[self.rayon.idRayon]))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'monApp/delete_rayon.html')
    
    def test_rayon_delete_view_post(self):
        response = self.client.post(reverse('delete_rayon', args=[self.rayon.idRayon]))
